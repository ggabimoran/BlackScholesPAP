...
namespace master{
  FDM::FDM(unsigned long M,unsigned long N)
  ...
  FDMImplicit::FDMImplicit(unsigned long M,unsigned long N,const ReducedBlackScholesPDE& rpde)
  ...
  void FDMImplicit::initialize_matrices(){
    //initialization of A
    for (int i=0;i<pA->get_nl();i++){
      (*pA)(i,i) = 1-2*lambda_;
      if (i>0) (*pA)(i-1,i) = -lambda_;
      if (i<(pA->get_nl()-1)) (*pA)(i+1,i) = -lambda_;
    }
    //initialization of u^n+1
    for (int j=0;j<pold_result->get_nl();j++) {
      (*pold_result)(j)=ppde_->terminal_boundary(minx_+(j+1)*dx_);
    }
    //initialization of b
  ...
  FDMCrankNicholson::FDMCrankNicholson(unsigned long M,unsigned long N,const CompleteBlackScholesPDE& rpde)
  ...
  void FDMCrankNicholson::initialize_matrices(){
    ...
    //initialization of u^n+1
    for (int k=0;k<pold_result->get_nl();k++) {
      (*pold_result)(k)=ppde_->terminal_boundary(minx_+(k+1)*dx_);
    }
    //initialization of b
